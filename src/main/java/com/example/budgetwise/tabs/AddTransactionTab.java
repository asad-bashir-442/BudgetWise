...
/**
 * This class allows the user to add a transaction
 */
public class AddTransactionTab extends Tab {
    ...
    private AddTransactionTab(){
        ...
        addTransactionBtn.setOnAction(event -> {
            ...
            if(amountField.getText().length() >=1 && textArea.getText().length() >=1 && date != null && isNumeric(amountField.getText()) && account != null && category != null){
    ...
    public void refresh(){
        ...
        if (CategoryTable.getInstance().getAllCategories().size()>=1){
            categoryComboBox.getSelectionModel().selectFirst();
        }else{
            categoryComboBox.setPlaceholder(new Text("Please create an category"));
        }
    }


    /**
     * This method checks if the string given is a number or not
     * @param str
     * @return
     */
